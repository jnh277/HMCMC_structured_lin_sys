...
N = 200
theta = 0.5
U = 0.9         # the uneven limits really demonstrates benefit of conditional mean
L = 0.4
...
save_file = Path("./trunc_normal_model_std.pkl")
if save_file.is_file():
    stan_model = pickle.load(open('trunc_normal_model_std.pkl', 'rb'))
else:
    # compile stan model
    stan_model = ps.StanModel(file="truncated_normal_known_std.stan")
    # save compiled file
    # save it to the file 'trunc_normal_model.pkl' for later use
    with open('trunc_normal_model_std.pkl', 'wb') as f:
